...
export function GitHubAuthButton({ mode = "login" }: GitHubAuthButtonProps) {
  ...
  return (
    <button
      ...
      className="h-10 w-10 rounded-full border border-border bg-card hover:bg-accent text-foreground flex items-center justify-center transition-all hover:scale-105 active:scale-95 shadow-sm focus:outline-none focus:ring-2 focus:ring-primary/50 disabled:opacity-50 cursor-pointer"
    >
      {loading ? (
